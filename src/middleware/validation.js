...
const jobSchemas = {
  createJob: Joi.object({
    body: Joi.object({
      ...
      details: Joi.alternatives().try(Joi.string().min(5).max(2000), Joi.object()).optional().allow('', null),
      budget: Joi.number().min(1000).required(),
      date_preference: Joi.string().valid('on_date', 'before_date', 'flexible').required(),
      preferred_date: Joi.date().iso().min('now').optional().allow(null),
      deadline_date: Joi.date().iso().min('now').optional().allow(null),
      time_preference: Joi.string().valid('morning', 'midday', 'afternoon', 'evening', 'flexible').optional().allow('', null),
  ...
  updateJob: Joi.object({
    body: Joi.object({
      ...
      city: Joi.string().optional().allow('', null),
      address: Joi.string().optional().allow('', null),
      location: Joi.alternatives().try(Joi.string(), Joi.object()).optional().allow('', null),
    }),
  }),
